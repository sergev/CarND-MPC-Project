 ...
 *         p. 27-37 (2001).
 *
 * Both the original Jones algorithm and the Gablonsky modified version
 * are implemented and available.  The Gablonsky version
 ...
 * constraints", i.e. arbitrary nonlinear constraints.
 * A hidden constraint is represented by returning NaN (or Inf, or
...
#define nlopt_stop_time_(starttime, maxtime) 0
...
/* |               scaling and unscaling the vector x by DIRinfcn.         | */
/* |                                                                       | */
/* |          n -- An int. The dimension of the problem.               | */
...
/* |                                                                       | */
/* +-----------------------------------------------------------------------+ */
static void direct_dirinfcn_(fp fcn, double *x, double *c1,
    double *c2, int *n, double *f, int *flag__, void *fcn_data)
{
...
/* +-----------------------------------------------------------------------+ */
static void direct_dirsamplef_(double *c__, int *arrayi, double
    *delta, int *sample, int *new__, int *length,
    FILE *logfile, double *f, int *free, int *maxi,
    int *point, fp fcn, double *x, double *l, double *
    minf, int *minpos, double *u, int *n, int *maxfunc,
    const int *maxdeep, int *oops, double *fmax, int *
    ifeasiblef, int *iinfesiblef, void *fcn_data, int *force_stop)
{
    /* System generated locals */
    int length_dim1, length_offset, c_dim1, c_offset, i__1, i__2;
    double d__1;

    /* Local variables */
    int i__, j, helppoint, pos, kret;
...
/* |                                                                       | */
/* +-----------------------------------------------------------------------+ */
static int direct_dirgetlevel_(int *pos, int *length, int *maxfunc, int
    *n, int jones)
{
    /* System generated locals */
    int length_dim1, length_offset, ret_val, i__1;
...
/* |         Changed if statement to prevent run-time errors.              |
                 | */
/* +-----------------------------------------------------------------------+ */
static void direct_dirchoose_(int *anchor, int *s, int *actdeep,
    double *f, double *minf, double epsrel, double epsabs, double *thirds,
    int *maxpos, int *length, int *maxfunc, const int *maxdeep,
    const int *maxdiv, int *n, FILE *logfile,
    int *cheat, double *kmax, int *ifeasiblef, int jones)
{
    /* System generated locals */
    int s_dim1, s_offset, length_dim1, length_offset, i__1;
    ...
    if (*ifeasiblef >= 1) {
        ...
        for (j = 0; j <= i__1; ++j) {
            if (anchor[j] > 0) {
                s[k + s_dim1] = anchor[j];
                s[k + (s_dim1 << 1)] = direct_dirgetlevel_(&s[k + s_dim1],
                    &length[length_offset], maxfunc, n, jones);
                goto L12;
            }
/* L1001: */
        }
...
/* +-----------------------------------------------------------------------+ */
static void direct_dirdoubleinsert_(int *anchor, int *s, int *
    maxpos, int *point, double *f, const int *maxdeep, int *
    maxfunc, const int *maxdiv, int *ierror)
{
    /* System generated locals */
    int s_dim1, s_offset, i__1;

    /* Local variables */
    int i__, oldmaxpos, pos, help, iflag, actdeep;
...
/* |                                                                       | */
/* +-----------------------------------------------------------------------+ */
static int direct_dirgetmaxdeep_(int *pos, int *length, int *maxfunc, int *n)
{
    /* System generated locals */
    int length_dim1, length_offset, i__1, i__2, i__3;
...
static int isinbox_(double *x, double *a, double *b, int *n, int *lmaxdim)
{
    /* System generated locals */
    int ret_val, i__1;
...
/* | replaced value.                                                       | */
/* +-----------------------------------------------------------------------+ */
static void dirresortlist_(int *replace, int *anchor,
    double *f, int *point, int *length, int *n, int *maxfunc,
    int *maxdim, const int *maxdeep, FILE *logfile, int jones)
{
    /* System generated locals */
    int length_dim1, length_offset, i__1;
...
/* +-----------------------------------------------------------------------+ */
static void direct_dirreplaceinf_(int *free, int *freeold,
    double *f, double *c__, double *thirds, int *length,
    int *anchor, int *point, double *c1, double *c2,
    int *maxfunc, const int *maxdeep, int *maxdim, int *n,
    FILE *logfile, double *fmax, int jones)
{
    /* System generated locals */
    int c_dim1, c_offset, length_dim1, length_offset, i__1, i__2, i__3;
    double d__1, d__2;

    /* Local variables */
    double a[32], b[32];
    int i__, j, k, l;
    double x[32], sidelength;
    int help;
...
/* +-----------------------------------------------------------------------+ */
static void dirinsert_(int *start, int *ins, int *point,
    double *f, int *maxfunc)
{
    /* System generated locals */
    int i__1;
...
/* +-----------------------------------------------------------------------+ */
static void direct_dirinsertlist_(int *new__, int *anchor, int *point,
    double *f, int *maxi, int *length, int *maxfunc,
    const int *maxdeep, int *n, int *samp, int jones)
{
    /* System generated locals */
    int length_dim1, length_offset, i__1;
...
/* +-----------------------------------------------------------------------+ */
static void dirinsertlist_2__(int *start, int *j, int *k,
    int *list2, double *w, int *maxi, int *n)
{
    /* System generated locals */
    int list2_dim1, list2_offset, i__1;
...
/* +-----------------------------------------------------------------------+ */
static void dirsearchmin_(int *start, int *list2, int *pos, int *k, int *n)
{
    /* System generated locals */
    int list2_dim1, list2_offset;
...
/* +-----------------------------------------------------------------------+ */
static void direct_dirsamplepoints_(double *c__, int *arrayi,
    double *delta, int *sample, int *start, int *length,
    FILE *logfile, double *f, int *free,
    int *maxi, int *point, double *x, double *l,
    double *minf, int *minpos, double *u, int *n,
    int *maxfunc, const int *maxdeep, int *oops)
{
    /* System generated locals */
    int length_dim1, length_offset, c_dim1, c_offset, i__1, i__2;
...
/* +-----------------------------------------------------------------------+ */
static void direct_dirdivide_(int *new__, int *currentlength,
    int *length, int *point, int *arrayi, int *sample,
    int *list2, double *w, int *maxi, double *f, int *maxfunc,
    const int *maxdeep, int *n)
{
    /* System generated locals */
    int length_dim1, length_offset, list2_dim1, list2_offset, i__1, i__2;
    double d__1, d__2;

    /* Local variables */
    int i__, j, k, pos, pos2;
    int start;
...
/* Computing MIN */
        ...
        w[j] = MIN(d__1,d__2);
        pos = point[pos];
        dirinsertlist_2__(&start, &j, &k, &list2[list2_offset], &w[1], maxi, n);
/* L10: */
    }
    ASRT(pos <= 0);
    i__1 = *maxi;
...
/* |    SUBROUTINE DIRGet_I                                                | */
/* +-----------------------------------------------------------------------+ */
static void direct_dirget_i__(int *length, int *pos, int *arrayi,
    int *maxi, int *n, int *maxfunc)
{
    /* System generated locals */
    int length_dim1, length_offset, i__1;
...
/* +-----------------------------------------------------------------------+ */
static void direct_dirinit_(double *f, fp fcn, double *c__,
    int *length, int *actdeep, int *point, int *anchor,
    int *free, FILE *logfile, int *arrayi,
    int *maxi, int *list2, double *w, double *x,
    double *l, double *u, double *minf, int *minpos,
    double *thirds, double *levels, int *maxfunc, const int *maxdeep,
    int *n, int *maxor, double *fmax, int *ifeasiblef,
    int *iinfeasible, int *ierror, void *fcndata,
    int jones, double starttime, double maxtime, int *force_stop)
{
    /* System generated locals */
    int c_dim1, c_offset, length_dim1, length_offset, list2_dim1,
...
/* |    Initialise the list.                                               | */
/* +-----------------------------------------------------------------------+ */
static void direct_dirinitlist_(int *anchor, int *free, int *point,
    double *f, int *maxfunc, const int *maxdeep)
{
    /* System generated locals */
    int i__1;
...
/* |                                                                       | */
/* |                                                                       | */
/* |       oops -- An int. If an upper bound is less than a lower      | */
/* |               bound or if the initial point is not in the             | */
/* |               hyper-box oops is set to 1 and iffco terminates.        | */
/* |                                                                       | */
/* +-----------------------------------------------------------------------+ */
static void direct_dirpreprc_(double *u, double *l, int *n,
    double *xs1, double *xs2, int *oops)
{
    /* System generated locals */
    int i__1;
...
static void direct_dirheader_(FILE *logfile, int *version,
    double *x, int *n, double *eps, int *maxf, int *maxt,
    double *l, double *u, int *algmethod, int *maxfunc,
    const int *maxdeep, double *fglobal, double *fglper,
    int *ierror, double *epsfix, int *iepschange,
    double *volper, double *sigmaper)
{
    /* System generated locals */
    int i__1;

    /* Local variables */
    int imainver, i__, numerrors, isubsubver, ihelp, isubver;
...
static void direct_dirsummary_(FILE *logfile, double *x,
    double *l, double *u, int *n, double *minf, double *fglobal,
    int *numfunc, int *ierror)
{
    /* Local variables */
    int i__;
...
/* +-----------------------------------------------------------------------+ */
static void direct_direct_(fp fcn, double *x, int *n,
    double *eps, double epsabs, int *maxf, int *maxt,
    double starttime, double maxtime, int *force_stop, double *minf, double *l,
    double *u, int *algmethod, int *ierror, FILE *logfile,
    double *fglobal, double *fglper, double *volper,
    double *sigmaper, void *fcn_data)
{
    /* System generated locals */
    int i__1, i__2;
    double d__1;
    ...
    int MAXFUNC = *maxf <= 0 ? 101000 : (*maxf + 1000 + *maxf / 2);
    int MAXDEEP = *maxt <= 0 ? MAXFUNC/5: *maxt + 1000;
    const int MAXDIV = 5000;

    /* Local variables */
    int increase;
    double *c__ = 0     /* was [90000][64] */,
           *f = 0       /* was [90000][2] */;
    int i__, j, *s = 0  /* was [3000][2] */, t;
    double *w = 0;
    double divfactor;
    int ifeasiblef, iepschange, actmaxdeep;
    int actdeep_div__, iinfesiblef;
    int pos1, newtosample;
    int ifree, help;
    double *oldl = 0, fmax;
    int maxi;
    double kmax, *oldu = 0;
    int oops, *list2 = 0    /* was [64][2] */, cheat;
    double delta;
    int mdeep, *point = 0, start;
    int *anchor = 0, *length = 0    /* was [90000][64] */, *arrayi = 0;
    double *levels = 0, *thirds = 0;
    double epsfix;
    int oldpos, minpos, maxpos, tstart, actdeep, ifreeold, oldmaxf;
    int numfunc = 0, version;
    int jones;
...
#else
#define MY_ALLOC(p, t, n) p = (t *) alloca(sizeof(t) * (n))
#endif
    ...
    MY_ALLOC(c__, double, MAXFUNC * (*n));
    MY_ALLOC(length, int, MAXFUNC * (*n));
    MY_ALLOC(f, double, MAXFUNC * 2);
    MY_ALLOC(point, int, MAXFUNC);
    if (*maxf <= 0) *maxf = MAXFUNC - 1000;

    MY_ALLOC(s, int, MAXDIV * 2);

    MY_ALLOC(anchor, int, MAXDEEP + 2);
    MY_ALLOC(levels, double, MAXDEEP + 1);
    MY_ALLOC(thirds, double, MAXDEEP + 1);
    if (*maxt <= 0) *maxt = MAXDEEP;

    MY_ALLOC(w, double, (*n));
    MY_ALLOC(oldl, double, (*n));
    MY_ALLOC(oldu, double, (*n));
    MY_ALLOC(list2, int, (*n) * 2);
    MY_ALLOC(arrayi, int, (*n));
...
/* +-----------------------------------------------------------------------+ */
        ...
        direct_dirchoose_(anchor, s, &MAXDEEP, f, minf, *eps, epsabs, levels, &maxpos, length,
            &MAXFUNC, &MAXDEEP, &MAXDIV, n, logfile, &cheat, &kmax, &ifeasiblef, jones);
/* +-----------------------------------------------------------------------+ */
...
/* +-----------------------------------------------------------------------+ */
                direct_dirsamplepoints_(c__, arrayi, &delta, &help, &start, length,
                    logfile, f, &ifree, &maxi, point, &x[1], &l[1],
                    minf, &minpos, &u[1], n, &MAXFUNC, &MAXDEEP, &oops);
                if (oops > 0) {
                    if (logfile)
                        fprintf(logfile, "WARNING: Error occured in routine DIRsamplepoints.\n");
                    *ierror = -4;
...
/* +-----------------------------------------------------------------------+ */
                direct_dirsamplef_(c__, arrayi, &delta, &help, &start, length,
                        logfile, f, &ifree, &maxi, point, fcn, &x[1], &l[1],
                        minf, &minpos, &u[1], n, &MAXFUNC, &MAXDEEP, &oops, &fmax,
...
/* Perform global minimization using (Gablonsky implementation of) DIRECT
   ...
   fglobal: the global minimum of f, if known ahead of time
       -- this is mainly for benchmarking, in most cases it
          is not known and you should pass -HUGE_VAL
   fglobal_reltol: relative tolerance on how close we should find fglobal
       -- ignored if fglobal is -HUGE_VAL
    ...
    double        xtol_rel)         // relative x tolerance (stop range), like 1e-3
{
    ...
    double  volume_reltol  = xtol_rel * xtol_rel;
    double  sigma_reltol   = -1.0;
    int    *force_stop     = NULL;
    double  fglobal        = -HUGE_VAL;
    double  fglobal_reltol = 0.0;
    FILE   *logfile        = NULL;
    int     algmethod      = 0; // Original Jones algorithm
  //int     algmethod      = 1; // Gablonsky modified version
    int     ierror;
    ...
    direct_direct_(obj_func, x, &dimension, &magic_eps, magic_eps_abs,
                   &max_feval, &max_iter,
